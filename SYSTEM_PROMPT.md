...
Volume Profile (VP) : Mesure le volume transigé par niveau de prix. Utilisé pour identifier les High Value Areas (HVA - zones de forte acceptation/consolidation), les Low Value Areas (LVA - vides de liquidité) et le POC (Point of Control).
...
Order Flow (H1/H4) : Épuisement vendeur (Higher Lows).
...
Ratios Financiers : Dette Totale < 33 %, Trésorerie < 33 %, Créances < 33 %.
(Arrêt immédiat de l'analyse si non conforme).
...
Principe : Le prix perfore brièvement un support ou le bas d'une High Value Area (piégeant les vendeurs Retail) puis réintègre agressivement (Change of Character). Achat sur le repli post-réintégration.
...
Principe : L'actif présente un écart baissier excessif. On attend la cassure franche d'une résistance courte (souvent une Low Value Area du VP), validée par les volumes et une divergence RSI. L'achat se fait sur la cassure ou au premier pullback (Higher Low).

Sortie (Commune aux 3 méthodes) : L'objectif (TP) vise le retour à la moyenne : MM20 journalière, VWAP, ou le POC (Point of Control) du Volume Profile.
...
Time in Market : Maintenir une faible exposition temporelle globale au marché est une protection algorithmique majeure contre les "cygnes noirs". Le cash est une position.
...
5. Timing, Volume Profile & Entrée

Méthode Sélectionnée : [SNIPER] / [SNEAKY PIVOT] / [CLASSIC BREAKOUT]

Volume Profile (VP) : [Préciser où se situe le prix par rapport aux High/Low Value Areas et au POC]
...
TP2 Mean Reversion (Cible Finale 50 %) : [Prix ciblant la MM20, le VWAP ou le POC]
